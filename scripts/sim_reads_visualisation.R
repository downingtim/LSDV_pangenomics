...
vcf_path <- function(method, sample) {
  file.path( method, "BCF_VCF_FILES",
    paste0(sample, ".norm.bcf.vcf.gz") )}
...
results <- map_dfr(seq_len(nrow(samples)), function(i) {
  ...
  sample1 = str_split_1(sample, "_")[2] # sim_KX_100
  value=0
  if(sample1 == "OQ511520"){  value=39}
  if(sample1 == "KX764645"){  value=40}
  truth0 <- read_snps(vcf_path("../LSDV1", samples$sample[value]))
  truth1 <- read_snps(vcf_path("../LSDVG", samples$sample[value]))
  truth2 <- read_snps(vcf_path("../LSDVG_3", samples$sample[value]))
  truth3 <- read_snps(vcf_path("../LSDVG_6", samples$sample[value]))
  truth <- bind_rows(truth0, truth1, truth2, truth3) %>% distinct()

  bind_rows(    compare_sets(      truth, lsdv1,
      "Linear", sample) %>% mutate(comparison = "Linear"), 
    ...
    compare_sets(      truth, lsdvg_6_m, "Giraffe_PVG16", sample
    ) %>% mutate(comparison = "Giraffe-PVG16")
  ) %>% mutate(  sample = sample,depth = depth,   ref = ref ) })
str(results)
...
recall2 <- ggplot( pivot_longer( subset(results, comparison!="Giraffe-PVG1"),
  ...
  geom_smooth(se = FALSE, linewidth = 1.5, method = "loess", alpha=0.4) +
   ...
   scale_y_continuous(limits = c(0.85, 1)) + theme_bw() +
  labs(x = "Read depth", y = "Recall") +
  guides(linetype = "none") +guides(colour = "none") +
  scale_color_manual(values = c("Linear" = "red", "Giraffe-PVG13" = "blue",
                                "Giraffe-PVG16" = "cyan"))
...
precision2 <- ggplot( pivot_longer( subset(results, comparison!="Giraffe-PVG1"),
  ...
  geom_smooth(se = FALSE, linewidth = 1.5, method = "loess", alpha=0.4) +
   scale_x_continuous(limits = c(80, 200),  breaks = seq(80, 200, 20)) +
   scale_y_continuous(limits = c(0.99, 1)) + theme_bw() +
  labs(x = "Read depth", y = "Precision") +
  guides(linetype = "none") +guides(colour = "none") +
  scale_color_manual(values = c("Linear" = "red", "Giraffe-PVG13" = "blue",
                                "Giraffe-PVG16" = "cyan"))
...
F1_2 <- ggplot( pivot_longer( subset(results, comparison!="Giraffe-PVG1"),
  ...
  geom_smooth(se = FALSE, linewidth = 1.5, method = "loess", alpha=0.4) +
   scale_x_continuous(limits = c(80, 200),  breaks = seq(80, 200, 20)) +
   scale_y_continuous(limits = c(0.968, 1)) + theme_bw() +
  labs(x = "Read depth", y = "F1 score") +
  guides(linetype = "none") +guides(colour = "none") +
  scale_color_manual(values = c("Linear" = "red", "Giraffe-PVG13" = "blue",
                                "Giraffe-PVG16" = "cyan"))
...
ggplot( pivot_longer(
  ...
  scale_y_continuous(limits = c(0.6, 1)) + theme_bw() +
  labs(x = "Read depth", y = "Fraction of all SNPs")
dev.off()
