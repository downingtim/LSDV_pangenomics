# LSDV pangenome variation graph (PVG) read mapping and mutation detection
...
This bash script (run_main.sh) supports three different read mapping approaches and multiple reference genome configurations, providing researchers with flexible options for variant discovery and genomic analysis.
The script 'run_main.sh' can do read mapping for Minimap2, Giraffe and VG-MAP.
The script 'run_Giraffe.sh' can do read mapping for Giraffe only.
The script 'run_Minimap2.sh' can do read mapping for Minimap2 only.
The script 'run_VG-MAP.sh' can do read mapping for VG-MAP only.
